...
<div class="main">
	...
	<p class="short-input ue-clear">
    	<label>密码：</label>
        <input name="password" type="text" placeholder="密码" />注：密码不做修改
    </p>
    <p class="short-input ue-clear">
    	<label>姓名：</label>
    ...
    <div class="short-input select ue-clear">
        ...
        <select name="dept_id">
        	<option value="-1">请选择</option>
        	   ...
        	   <?php if(is_array($depts)): $i = 0; $__LIST__ = $depts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vol["id"]); ?>" <?php if($data.dept_id==$vol.id): ?>selected="selected"<?php endif; ?>><?php echo ($vol["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	...
	<p class="short-input ue-clear">
    	<label>生日：</label>
        <input name="birthday" value="<?php echo ($data["birthday"]); ?>" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
    </p>
	<p class="short-input ue-clear">
    	<label>联系电话：</label>
